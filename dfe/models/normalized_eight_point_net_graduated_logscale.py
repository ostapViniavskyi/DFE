...
class NormalizedEightPointNet(nn.Module):
    ...
    def __init__(self, depth=1, side_info_size=0):
        ...
        self.min_scale_multiplier = math.log(0.25)
        self.max_scale_multiplier = math.log(0.95)
    ...
    def forward(self, pts, side_info):
        ...
        log_scale = torch.log(torch.max(residual, dim=-1, keepdim=True)[0])
        out = []
        ...
        for _ in range(self.depth):
            input_p_s_w_r = torch.cat(
                (
                    ...
                    log_scale.detach().exp()[..., None].expand(-1, -1, weights.shape[-1])
                ),
                dim=1
            )
            log_scale_multiplier = self.weights_iter(input_p_s_w_r)
            log_scale_multiplier = torch.sigmoid(log_scale_multiplier) * (
                self.max_scale_multiplier - self.min_scale_multiplier) + self.min_scale_multiplier
            # determine next scale
            log_scale = log_scale.detach() + log_scale_multiplier
            # print(f'Iter {_}: {scale[0]}')
            weights = self.kernel.get_weight(residual, scale=log_scale.exp()).unsqueeze(1)
